...
export class ProfessionalModel {
  ...
  static async insertProfessional(id_usuario, especialidades) {
    ...
    try {
      ...
      throw new InternalServerError('Error al crear el profesional.');
    }
  }
  ...
  static async getProfessional(id_profesional) {
    try {
      ...
      const specialitiesPromise = specialities.rows.map(async (speciality) => {
        const response = await pool.query(
          `
            SELECT *
            FROM especialidad
      ...
      throw new InternalServerError('Error al obtener el profesional.');
    }
  }
  //! Ver si hace falta refactorizar
  static async getAllProfessionals() {
    try {
      const professionals = await pool.query(`SELECT * FROM profesional`);
      ...
      throw new InternalServerError('Error al obtener los profesionales.');
    }
  }
}
